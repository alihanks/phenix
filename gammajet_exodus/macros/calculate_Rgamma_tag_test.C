...
void calculate_Rgamma_tag_test(){
  ...
  for(int i=0; i<4; i++){
    ...
    cout << "etag " << (Ninc[i]-Nleft[i])/Ninc[i] <<endl;

    float etag=(Ninc[i]-Nleft[i])/Ninc[i]+(1.0-sep_eff)/Rgamma*0.74;

    cout << "corrected etag " << etag <<endl;

    
    Rtag[i]=(1.0-etag)/((Nfl[i]/Ntag[i]-1.0)*etag+1.0/Rgamma);
    ...
    //Rtag[i]=(1-etag)/((Nfl[i]/Ntag[i]-1.0)*etag+1.0/Rgamma);

    Rtagerr[i]=Rerr*(1.0-etag)/((Nfl[i]/Ntag[i]-1.0)*Rgamma*etag+1.0)/((Nfl[i]/Ntag[i]-1.0)*Rgamma*etag+1.0);

    Rtagsys[i]=Rerrcorr*(1.0-etag)/((Nfl[i]/Ntag[i]-1.0)*Rgamma*etag+1.0)/((Nfl[i]/Ntag[i]-1.0)*Rgamma*etag+1.0);


    efferr[i]=eff[i]*sqrt((Rtagerr[i]/Rtag[i])*(Rtagerr[i]/Rtag[i])+(Rerrcomb/Rgamma)*(Rerrcomb/Rgamma));


    cout << "rel Rgamma err: " << Rerr/Rgamma << " Rgamma err Tagged: " << Rtagerr[i]/Rtag[i] <<endl;

    cout << "rel sys Rgamma err: " << Rerrcorr/Rgamma << " Rgamma err Tagged: " << Rtagsys[i]/Rtag[i] <<endl;


    cout << "scale rel error by: " << 1.0/((Nfl[i]/Ntag[i]-1.0)*Rgamma*etag+1.0) <<endl;

    cout << "scaled rel error: " << 1.0/((Nfl[i]/Ntag[i]-1.0)*Rgamma*etag+1.0)*Rerrcorr/Rgamma <<endl;

    cout << "no false rel error: " << 1.0/(-1.0*Rgamma*etag+1.0) <<endl;

    cout << "no false 50% rel error: " << 1.0/(-1.0*.90+1.0) <<endl;
 ...
 const char* outfile = "Rgamma_tag_test.root";
