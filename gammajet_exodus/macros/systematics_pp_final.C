void systematics_pp_final(){
  ...
  TFile *fh = new TFile("cocktail_10M_pp200_run3_bias_final_high.root");
  TH1D *high = (TH1D*) fh->Get("pteR");
  high->SetLineColor(2);

  TFile *fs = new TFile("cocktail_10M_pp200_run3_bias_final_low.root");
  TH1D *low = (TH1D*) fs->Get("pteR");
  low->SetLineColor(2);

  TFile *fc = new TFile("cocktail_10M_pp200_run3_bias_final.root");
  ...
  sys_lo_direct->Add(pteRGamma,-0.5);
  sys_lo_direct->Add(pteRCGamma,-0.5);
  ...
  sys_hi_direct->Add(pteRGamma,0.5);
  sys_hi_direct->Add(pteRCGamma,0.5);
  ...
  sys_total->Draw("same");
  TF1 *fit_p2 = new TF1("fit_p2","13.2-1.1*x+0.11*x*x",0.,5.);
