void systematics_pp_ppfinal(){
  ...
  TFile *fh = new TFile("cocktail_10M_pp200_run3_ppfinal_high.root");
  TH1D *high = (TH1D*) fh->Get("pteR");
  high->SetLineColor(2);

  TFile *fs = new TFile("cocktail_10M_pp200_run3_ppfinal_low.root");
  TH1D *low = (TH1D*) fs->Get("pteR");
  low->SetLineColor(2);

  TFile *fc = new TFile("cocktail_10M_pp200_run3_ppfinal.root");
  ...
  sys_total->Draw("same");
  TF1 *fit_p2 = new TF1("fit_p2","13.6-1.5*x+0.19*x*x",0.,5.);
