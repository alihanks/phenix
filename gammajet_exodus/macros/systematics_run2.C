void systematics_run2(){
  ...
  TH2F *hist_background = new TH2F("hist_background","Non-photonic electron invariant cross-section",10,0.,4.5,10,0.,1.05);
  ...
  TFile *fh = new TFile("cocktail_10M_pp200_run2_hard.root");
  TH1D *hard = (TH1D*) fh->Get("pteR");
  hard->Add(pteRKe3,-.25);
  hard->SetLineColor(2);
  //  hard->Draw("chist");

  TFile *fc = new TFile("cocktail_10M_pp200_final.root");
  ...
  sys_direct->Add(pteRGamma,-1.);
  sys_direct->Add(pteRCGamma,-1.);
