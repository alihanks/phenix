...
void tag_correction(){
  ...
  TFile *fshark_exodus=new TFile("/phenix/hp/data10/mjuszkie/exodus/merged/all.root");
  ...
  */
  ...
  for(int iptpi=81; iptpi<PTpi_PTgam_miss->GetNbinsX(); iptpi++){
    ...
    for(int iptgam=1; iptgam<PTpi_PTgam_miss->GetNbinsY(); iptgam++){
      ...
      if(ntot>0) PTpi_PTgam_miss->SetBinContent(iptpi,iptgam,(bctr)/ntot);
      else PTpi_PTgam_miss->SetBinContent(iptpi,iptgam,0.0);

      if(ntot>0) PTpi_PTgam_false->SetBinContent(iptpi,iptgam,(bcfl)/ntot);
      else PTpi_PTgam_false->SetBinContent(iptpi,iptgam,0.0);
