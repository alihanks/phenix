		     ...
		     ParticlePropertyList *PPList)
{
  ...
  for ( int ipart=1; ipart<=PList->GetLength(); ipart++) 
	...
	if(mass>0.118 && mass<0.162) { inwindow=1;}
	...
	if(firedert ==1 && accept1> 0 && fabs(zemce) < 155 && ecoree>1.0){
	  ...
	  */
	  ...
	  //not using ert weighting and different ptcut
	...
	if(firedert==1&&accept1>0&&accept2>0&&ecoree>1&&ecorep>1){
	  ...
	  //here don't use the ert eff

	  RECONPI_ZEMC_PT->Fill(primzemc,primpt);
	  if(!inwindow && (fabs(zemcp) < 155 || fabs(zemce) < 155)) PTpi_ZEMCpi_PTgam_MISS->Fill(primpt,primzemc,pte);
	}
		...
		for(int gampart=1; gampart<nparts; gampart++)
		  {
		    ...
		    */
			...
			if(iparent2==iparent1){
			  ...
			  PTpi_ZEMCpi_PTgam_TRUE->Fill(pipt1,zemc1,pt1);
			}else{
			  if(parent2==111) PTpi_ZEMCpi_PTgam_PI0->Fill(pipt1,zemc1,pt1);
