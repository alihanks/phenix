		     ...
		     ParticlePropertyList *PPList)
{
  ...
  //For Run 7--meg from taxi
  char deadfile[]="/direct/phenix+u/workarea/mjuszkie/gammajet_exodus/livetowers_nothot_rg5.list";
  ...
  for ( int ipart=1; ipart<=PList->GetLength(); ipart++) 
	...
	if(firedert ==1 && accept2> 0 && fabs(zemcp) < 155 && ecorep>1.0){
	  ...
	  //Do I not tag photon 1 because I miss it's partner?
	  if(accept1==-9||pte<1.0) PTpi_ZEMCpi_PTgam_MISS->Fill(primpt,primzemc,pte);
	}
	//For miss sharkfin do I measure both photons but reconstruct the invmass outside our window?
	if(accept1>0&&accept2>0&&ecoree>1&&ecorep>1 && !intagwindow && (fabs(zemcp) < 155 || fabs(zemce) < 155)) PTpi_ZEMCpi_PTgam_MISS->Fill(primpt,primzemc,pte);	
		...
		if(ispi0>0)
		  {
		    ...
		    //if(falsepair && parent1==111) PTpi_ZEMCpi_PTgam_TRUE->Fill(pipt1,zemc1,pt1);

		    
		  }else{
		    //Not tagged  
		    if(parent1==111) PTpi_ZEMCpi_PTgam_MISS_PI0->Fill(pipt1,zemc1,pt1);
		    if(parent1==221) PTpi_ZEMCpi_PTgam_MISS_ETA->Fill(pipt1,zemc1,pt1);
	  ...
	  if(gamaccept[gamtrig]>0)
	      ...
	      for(int gampart=1; gampart<nparts; gampart++)
		{
		  ...
		  if(mass>0.12 && mass<0.165){ 
			...
			PTpi_ZEMCpi_PTgam_TRUE->Fill(pi0pt,zemc1,pt1); //changed pipt1 to pi0pt 2/20/09

		      }else{
			if(parent2==111){ 
			  PTpi_ZEMCpi_PTgam_PI0->Fill(pipt1,zemc1,pt1);
			  //RECONPI_ZEMC_PTTRUE_PTRECO_falsePI0->Fill(zemc1,pipt1,pi0pt);
			}
			if(parent2==221) PTpi_ZEMCpi_PTgam_ETA->Fill(pipt1,zemc1,pt1);
			if(parent2==-111 || parent2==22) PTpi_ZEMCpi_PTgam_DIR->Fill(pipt1,zemc1,pt1);
