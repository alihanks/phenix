  ...
  for ( int ipart=1; ipart<=PList->GetLength(); ipart++) 
  {
      ...
      if (pid == 111)
	{
	  realpi0pt->Fill(primpt, primfillweight);
 	  ...
 	  weight   = NextParticle->GetWeight();
 	  fillweight1 = abs_norm*weight/(0.5*n_pi0_generated);
 	  fillweight1 = fillweight1/0.025;
 	  mom1 = NextParticle->Get4mom();
	  theta = thetaof(mom1.Getp());
	  vtxz = NextParticle->GetzVertex();
	  ...
	  ye = log( (mom1.GetE()+pze)/(mom1.GetE()-pze) )/2.0;
	  ...
	  {
	      for(int j=0;j<6;j++)
		if(phenix_geom(j,(float)pxe,(float)pye,(float)pze,(float)vtxz,deadfile)) accept1=j;
	    }
	  ...
	  //	  cout << NextParticle->GetID() << "Accept1:" << accept1 << "Vtxz:" << vtxz << endl;
 	  ...
 	  weight   = NextParticle->GetWeight();
 	  fillweight2 = abs_norm*weight/(0.5*n_pi0_generated);
 	  fillweight2 = fillweight2/0.025;
 	  mom2 = NextParticle->Get4mom();
	 
	  theta = thetaof(mom2.Getp());
	  vtxz = NextParticle->GetzVertex();

 	  pp  = sqrt(mom2.GetE()*mom1.GetE()-mom2*mom2);
 	  pxp = mom2.Getp().Getpx();
 	  pyp = mom2.Getp().Getpy();
 	  pzp = mom2.Getp().Getpz();
 	  ptp = sqrt(pxp*pxp+pyp*pyp);

	  yp = log( (mom2.GetE()+pzp)/(mom2.GetE()-pzp) )/2.0;
	  accept2 = -1;
	  type2 = gRandom->Rndm();

	  //openning angle of two pion-photons
	  opangle = (mom1.Getp()*mom2.Getp()) /
	    (sqrt(mom1.Getp()*mom1.Getp())*
	     sqrt(mom2.Getp()*mom2.Getp()));
	  ...
	  if(fabs(yp)<=0.35)
	    {
	      for(int j=0;j<6;j++)
		if(phenix_geom(j,(float)pxp,(float)pyp,(float)pzp,(float)vtxz,deadfile)) accept2=j;
	    }
