  ...
  for ( int ipart=1; ipart<=PList->GetLength(); ipart++) 
	...
	if(firedert ==1 && accept1> 0 && fabs(zemce) < 155 && ecoree>1.0){
	  ...
	  */
	  ...
	  if(accept2<0||ecorep<1.0||sume12<4.0 || passasym==0){ 
	    PTpi_ZEMCpi_PTgam_MISS->Fill(primpt,primzemc,pte,(float)accept1);
	...
	if(firedert ==1 && accept2> 0 && fabs(zemcp) < 155 && ecorep>1.0){
	  ...
	  */
	  ...
	  if(accept1<0||ecoree<1.0||sume12<4.0 || passasym==0){ 
	    PTpi_ZEMCpi_PTgam_MISS->Fill(primpt,primzemc,pte,(float)accept1);
	    //if(accept2==-9||ptp<0.5) PTpi_ZEMCpi_PTgam_MISS->Fill(primpt,primzemc,pte,(float)accept1);
	  }
