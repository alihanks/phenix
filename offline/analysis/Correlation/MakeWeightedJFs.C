...
void MakeWeightedJFs::MakeDphiFrom3D(TH1F* trigpt, int cbin)
{
	...
	for(int it = 0; it < NTRIGBIN; it++){
		...
		for(int ih = 0; ih < NPARTBIN; ih++){
			...
			MakeJetFunction(isdAu, 0, dphi_1d[cbin][it][ih], dphi_1d_mix[cbin][it][ih], corr[cbin][it][ih], ntrig[it], it, ih, cbin, 0.9, 1.4);
			MakeJetFunction(isdAu, 0, dphi_1d[cbin][it][ih], dphi_1d_mix[cbin][it][ih], corr_sys[cbin][it][ih], ntrig[it], it, ih, cbin, 1.0, 1.2);
			outfile->cd();
...
void MakeWeightedJFs::MakeDphiFrom2D(TH1F* trigpt, int cbin)
{
	...
	for(int it = 0; it < NTRIGBIN; it++){
		...
		for(int ih = 0; ih < NPARTBIN; ih++){
			...
			MakeJetFunction(isdAu, 1, dphi_1d[cbin][it][ih], dphi_1d_mix[cbin][it][ih], corr[cbin][it][ih], ntrig[it], it, ih, cbin, 0.9, 1.4);
			MakeJetFunction(isdAu, 1, dphi_1d[cbin][it][ih], dphi_1d_mix[cbin][it][ih], corr_sys[cbin][it][ih], ntrig[it], it, ih, cbin, 1.0, 1.2);
			outfile->cd();
			corr[cbin][it][ih]->Write();
		}
	}
